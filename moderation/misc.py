...
blocked_word = ['NIGGA', 'NIGGER', "NIGG", "REGIN",
                "IMAGPX", "REGGIN", "FAGGOT", "RETARD", "CUNT", "OOOL"]
        ...
        if banned == None:
            ...
            try:
                ...
                def check(ctx):
                    ...
                    # and ctx.content.startswith()!="p!say"
                ...
                try:
                    ...
                    await ctx.channel.send("Oh Well, I did not get the reason in time, too bad i can't ban without reason :(")
                    return
...
async def unban(ctx):
    ...
    if member.guild_permissions.ban_members:
        ...
        if banned == None:
            ...
            await ctx.channel.send(embed=embed)
            await punish_log(embed=embed)
            await ctx.guild.unban(banned, reason=f"Unbanned by {member}")
...
async def unban_service(ctx):
    ...
    if member.guild_permissions.mute_members:
        ...
        if banned == None:
            ...
            role_add = discord.utils.get(
                ctx.guild.roles, name="Service Blacklisted")
            role_remove = discord.utils.get(
                ctx.guild.roles, name="Can see services")
...
async def TempBan(ctx):
    ...
    if member.guild_permissions.kick_members:
        ...
        sec: str = sec[0]
        if str(sec) == "0":
            await ctx.channel.send(embed=await Invalid_Time_send())
            return
    ...
    if member.guild_permissions.mute_members:
        ...
        if reason == "":
            ...
            try:
                ...
                await ctx.channel.send("Oh Well, I did not get the reason in time, too bad i can't mute without reason :(")
                return
        await ctx.channel.send(f"Heres the time in calc in (sec): {sec} \n Reason: {reason}")
        done: bool = False
        while done == False:
        ...
        if warned == None:
            ...
            if reason == '':
                try:
                    ...
                    await ctx.channel.send("Oh Well, I did not get the reason in time, too bad i can't warn without reason :(")
                    return
            prepare = f'''SELECT MAX(`case_id`) FROM PunishmentLogs'''
            Cursor.execute(prepare)
            case_id = Cursor.fetchall()
    ...
    if member.guild_permissions.kick_members:
        ...
        if reason == "":
            ...
            try:
                ...
                await ctx.channel.send("Oh Well, I did not get the reason in time, too bad i can't service ban without reason :(")
                return
        await ctx.channel.send(f"Heres the time in calc in (sec): {sec} \n Reason: {reason}")
        done: bool = False
        while done == False:
